...
def insert_abr(key, abr):
    ...
    if val_abr(abr) < key[0] :
        if not(left_tree_abr(abr) is None) :
            insert_abr(key,left_tree_abr(abr))
        else :
            abr[0] = create_abr(key,abr)
            
    else :
         if not(right_tree_abr(abr) is None) :
...
def find_minimum_key_abr(abr):
    ...
    if not(abr[0] is None) :
        find_minimum_key_abr(left_tree_abr(abr))
    else :
        return key_abr(abr)
...
def find_maximum_key_abr(abr):
    ...
    if not(abr[2] is None) :
        find_minimum_key_abr(right_tree_abr(abr))
    else :
        return key_abr(abr)
...
def find_abr(value, abr):
    ...
    if val_abr(abr) < value :
        if not(left_tree_abr(abr) is None) :
            find_abr(value,left_tree_abr(abr))
        else :
            return None
...
def find_minimum_subtree_abr(sub_abr):
    ...
    return find_abr(val_abr(find_minimum_key_abr(sub_abr)), sub_abr)
...
def list_of_keys_abr(abr):
    ...
    def rech_abr(abr):
        ...
        if left_tree_abr(abr) != None:
            listKeys.append(rech_abr(left_tree_abr(abr)))
            
        listKeys.append(key_abr(abr))
        
        if right_tree_abr(abr) != None:
            listKeys.append(rech_abr(right_tree_abr(abr)))
...
def find_next_abr(sub_abr):
    ...
    def next_abr(value, abr):
        ...
        elif parent_abr(abr) != None:
            if val_abr(parent_abr(abr)) > value:
                return parent_abr(abr)
            else:
                return next_abr(value, abr)

    return next_abr(key_abr(sub_abr), sub_abr)
...
def delete_val_abr(val, abr):
    ...
    def delete_key_subabr_abr(sub_abr):
        ...
        minimum = find_minimum_subtree_abr(sub_abr)
        sub_abr[1] = key_abr(minimum)
        parent_abr(minimum)[0] = right_tree_abr(minimum)

    if abr[2] != None :
        return delete_key_subabr_abr(find_abr(val, abr))
    else :
        abr[0][3] = None
        return abr[0]
...
def find_next_key_abr(val, abr):
    ...
    return key_abr(find_next_abr(find_abr(val, abr)))
