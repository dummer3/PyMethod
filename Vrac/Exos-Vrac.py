...
def zipe_listes(liste_1, liste_2):
    ...
    return [(liste_1[i],liste_2[i]) for i in range(min(len(liste_1),len(liste_2)))];
    pass
...
def affiche_contenu_liste(liste_info):
    ...
    for i in liste_info:
        if i[2]:
            ...
            s = "M."
        print(f"{s} {i[0][1]} {i[0][0]} a {i[1]} ans.")
...
def extrait_liste_prénoms(liste_info):
    ...
    return [x[0][1] for x in liste_info];
    pass
...
def inverse_a_et_b(mot):
    """Retourne la chaine de caractères qui est la chaine d'entrée
...
def look_and_say(z, k=1):
    ...
    return z
    pass
teste_fonction("111221",look_and_say,"1",4);
teste_fonction("22",look_and_say,"22",4);
